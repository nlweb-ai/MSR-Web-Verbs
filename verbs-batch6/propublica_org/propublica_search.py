"""Auto-generated – ProPublica – Investigative Article Search – Query: "government spending investigation" """
...
class ArticleRequest:
    query: str = "government spending investigation"
    max_results: int = 5
...
def propublica_search(page: Page, request: ArticleRequest) -> ArticleResult:
    print(f"  Query: {request.query}\n")
    from urllib.parse import quote_plus
    url = f"https://www.google.com/search?q=site%3Apropublica.org+{quote_plus(request.query)}"
    print(f"Loading {url}...")
    checkpoint("Google site search for ProPublica")
    page.goto(url, wait_until="domcontentloaded"); page.wait_for_timeout(3000)
    data = page.evaluate(r"""(m) => {
        const r = [], s = new Set();
        for (const h of document.querySelectorAll('h3')) {
            if (r.length >= m) break;
            let t = h.innerText.trim().replace(/\s*[\|\u2013\u2014-]\s*ProPublica.*$/i, '').trim();
    ...
    print("\n" + "=" * 60)
    print(f"ProPublica: {request.query}")
    print("=" * 60)
...
def test_func():
    port = get_free_port(); profile_dir = get_temp_profile_dir("propublica_org")
    ...
    with sync_playwright() as pw:
        ...
        try:
            result = propublica_search(page, ArticleRequest())
