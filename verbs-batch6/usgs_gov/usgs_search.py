"""Auto-generated – USGS – Geological Report Search – Query: "earthquake California recent" """
...
class ReportRequest:
    query: str = "earthquake California recent"
    max_results: int = 5
...
def usgs_search(page: Page, request: ReportRequest) -> ReportResult:
    print(f"  Query: {request.query}\n")
    from urllib.parse import quote_plus
    url = f"https://www.google.com/search?q=site%3Ausgs.gov+{quote_plus(request.query)}"
    print(f"Loading {url}...")
    checkpoint("Google site search for USGS")
    page.goto(url, wait_until="domcontentloaded"); page.wait_for_timeout(3000)
    data = page.evaluate(r"""(m) => {
        const r = [], s = new Set();
        for (const h of document.querySelectorAll('h3')) {
            if (r.length >= m) break;
            let t = h.innerText.trim().replace(/\s*[\|\u2013\u2014-]\s*USGS|U\.S\. Geological Survey.*$/i, '').trim();
    ...
    items = [Report(**d) for d in data]
    result = ReportResult(items=items[:request.max_results])
    print("\n" + "=" * 60)
    print(f"USGS: {request.query}")
    print("=" * 60)
...
def test_func():
    port = get_free_port(); profile_dir = get_temp_profile_dir("usgs_gov")
    ...
    with sync_playwright() as pw:
        ...
        try:
            result = usgs_search(page, ReportRequest())
