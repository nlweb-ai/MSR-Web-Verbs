"""Auto-generated – Swappa – Used Device Search – Query: "iPhone 15 Pro Max" """
...
def swappa_search(page: Page, request: ListingRequest) -> ListingResult:
    print(f"  Query: {request.query}\n")
    from urllib.parse import quote_plus
    url = f"https://www.google.com/search?q=site%3Aswappa.com+{quote_plus(request.query)}"
    print(f"Loading {url}...")
    checkpoint("Google site search for Swappa")
    page.goto(url, wait_until="domcontentloaded"); page.wait_for_timeout(3000)
    data = page.evaluate(r"""(m) => {
        const r = [], s = new Set();
        for (const h of document.querySelectorAll('h3')) {
            if (r.length >= m) break;
            let t = h.innerText.trim().replace(/\s*[\|\u2013\u2014-]\s*Swappa.*$/i, '').trim();
...
def test_func():
    port = get_free_port(); profile_dir = get_temp_profile_dir("swappa_com")
    ...
    with sync_playwright() as pw:
        ...
        try:
            result = swappa_search(page, ListingRequest())
