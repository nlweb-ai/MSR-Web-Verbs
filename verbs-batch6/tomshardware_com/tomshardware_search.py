"""Auto-generated – Tom's Hardware – Hardware Review Search – Query: "best gaming motherboard 2024" """
...
class ReviewRequest:
    query: str = "best gaming motherboard 2024"
    max_results: int = 5
...
def tomshardware_search(page: Page, request: ReviewRequest) -> ReviewResult:
    print(f"  Query: {request.query}\n")
    from urllib.parse import quote_plus
    url = f"https://www.google.com/search?q=site%3Atomshardware.com+{quote_plus(request.query)}"
    print(f"Loading {url}...")
    checkpoint("Google site search for Tom's Hardware")
    page.goto(url, wait_until="domcontentloaded"); page.wait_for_timeout(3000)
    data = page.evaluate(r"""(m) => {
        const r = [], s = new Set();
        for (const h of document.querySelectorAll('h3')) {
            if (r.length >= m) break;
            let t = h.innerText.trim().replace(/\s*[\|\u2013\u2014-]\s*Tom's Hardware|Tom.*$/i, '').trim();
    ...
    print("\n" + "=" * 60)
    print(f"Tom's Hardware: {request.query}")
    print("=" * 60)
...
def test_func():
    port = get_free_port(); profile_dir = get_temp_profile_dir("tomshardware_com")
    ...
    with sync_playwright() as pw:
        ...
        try:
            result = tomshardware_search(page, ReviewRequest())
