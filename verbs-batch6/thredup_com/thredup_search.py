"""Auto-generated – ThredUp – Secondhand Fashion Search – Query: "women dresses summer" """
...
class ListingRequest:
    query: str = "women dresses summer"
    max_results: int = 5
...
def thredup_search(page: Page, request: ListingRequest) -> ListingResult:
    print(f"  Query: {request.query}\n")
    from urllib.parse import quote_plus
    url = f"https://www.google.com/search?q=site%3Athredup.com+{quote_plus(request.query)}"
    print(f"Loading {url}...")
    checkpoint("Google site search for ThredUp")
    page.goto(url, wait_until="domcontentloaded"); page.wait_for_timeout(3000)
    data = page.evaluate(r"""(m) => {
        const r = [], s = new Set();
        for (const h of document.querySelectorAll('h3')) {
            if (r.length >= m) break;
            let t = h.innerText.trim().replace(/\s*[\|\u2013\u2014-]\s*ThredUp|thredUp.*$/i, '').trim();
...
def test_func():
    port = get_free_port(); profile_dir = get_temp_profile_dir("thredup_com")
    ...
    with sync_playwright() as pw:
        ...
        try:
            result = thredup_search(page, ListingRequest())
