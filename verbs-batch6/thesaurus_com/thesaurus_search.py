"""Auto-generated – Thesaurus.com – Synonym Search – Query: "synonyms for happy" """
...
def thesaurus_search(page: Page, request: WordRequest) -> WordResult:
    print(f"  Query: {request.query}\n")
    from urllib.parse import quote_plus
    url = f"https://www.google.com/search?q=site%3Athesaurus.com+{quote_plus(request.query)}"
    print(f"Loading {url}...")
    checkpoint("Google site search for Thesaurus.com")
    page.goto(url, wait_until="domcontentloaded"); page.wait_for_timeout(3000)
    data = page.evaluate(r"""(m) => {
        const r = [], s = new Set();
        for (const h of document.querySelectorAll('h3')) {
            if (r.length >= m) break;
            let t = h.innerText.trim().replace(/\s*[\|\u2013\u2014-]\s*Thesaurus\.com|Thesaurus.*$/i, '').trim();
    ...
    print("\n" + "=" * 60)
    print(f"Thesaurus.com: {request.query}")
    print("=" * 60)
...
def test_func():
    port = get_free_port(); profile_dir = get_temp_profile_dir("thesaurus_com")
    ...
    with sync_playwright() as pw:
        ...
        try:
            result = thesaurus_search(page, WordRequest())
