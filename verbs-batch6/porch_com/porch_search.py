"""Auto-generated – Porch – Home Service Search – Query: "plumber near me reviews" """
...
class ServiceRequest:
    query: str = "plumber near me reviews"
    max_results: int = 5
...
def porch_search(page: Page, request: ServiceRequest) -> ServiceResult:
    print(f"  Query: {request.query}\n")
    from urllib.parse import quote_plus
    url = f"https://www.google.com/search?q=site%3Aporch.com+{quote_plus(request.query)}"
    print(f"Loading {url}...")
    checkpoint("Google site search for Porch")
    page.goto(url, wait_until="domcontentloaded"); page.wait_for_timeout(3000)
    data = page.evaluate(r"""(m) => {
        const r = [], s = new Set();
        for (const h of document.querySelectorAll('h3')) {
            if (r.length >= m) break;
            let t = h.innerText.trim().replace(/\s*[\|\u2013\u2014-]\s*Porch.*$/i, '').trim();
    ...
    items = [Service(**d) for d in data]
    result = ServiceResult(items=items[:request.max_results])
    print("\n" + "=" * 60)
    print(f"Porch: {request.query}")
    print("=" * 60)
...
def test_func():
    port = get_free_port(); profile_dir = get_temp_profile_dir("porch_com")
    ...
    with sync_playwright() as pw:
        ...
        try:
            result = porch_search(page, ServiceRequest())
