"""Auto-generated – RAWG – Video Game Search – Query: "best RPG games 2024" """
...
def rawg_search(page: Page, request: GameRequest) -> GameResult:
    print(f"  Query: {request.query}\n")
    from urllib.parse import quote_plus
    url = f"https://www.google.com/search?q=site%3Arawg.io+{quote_plus(request.query)}"
    print(f"Loading {url}...")
    checkpoint("Google site search for RAWG")
    page.goto(url, wait_until="domcontentloaded"); page.wait_for_timeout(3000)
    data = page.evaluate(r"""(m) => {
        const r = [], s = new Set();
        for (const h of document.querySelectorAll('h3')) {
            if (r.length >= m) break;
            let t = h.innerText.trim().replace(/\s*[\|\u2013\u2014-]\s*RAWG.*$/i, '').trim();
    ...
    items = [Game(**d) for d in data]
    result = GameResult(items=items[:request.max_results])
    print("\n" + "=" * 60)
    print(f"RAWG: {request.query}")
    print("=" * 60)
...
def test_func():
    port = get_free_port(); profile_dir = get_temp_profile_dir("rawg_io")
    ...
    with sync_playwright() as pw:
        ...
        try:
            result = rawg_search(page, GameRequest())
