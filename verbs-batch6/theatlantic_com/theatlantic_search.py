"""Auto-generated – The Atlantic – Magazine Article Search – Query: "artificial intelligence future society" """
...
class ArticleRequest:
    query: str = "artificial intelligence future society"
    max_results: int = 5
...
def theatlantic_search(page: Page, request: ArticleRequest) -> ArticleResult:
    print(f"  Query: {request.query}\n")
    from urllib.parse import quote_plus
    url = f"https://www.google.com/search?q=site%3Atheatlantic.com+{quote_plus(request.query)}"
    print(f"Loading {url}...")
    checkpoint("Google site search for The Atlantic")
    page.goto(url, wait_until="domcontentloaded"); page.wait_for_timeout(3000)
    data = page.evaluate(r"""(m) => {
        const r = [], s = new Set();
        for (const h of document.querySelectorAll('h3')) {
            if (r.length >= m) break;
            let t = h.innerText.trim().replace(/\s*[\|\u2013\u2014-]\s*The Atlantic.*$/i, '').trim();
    ...
    print("\n" + "=" * 60)
    print(f"The Atlantic: {request.query}")
    print("=" * 60)
...
def test_func():
    port = get_free_port(); profile_dir = get_temp_profile_dir("theatlantic_com")
    ...
    with sync_playwright() as pw:
        ...
        try:
            result = theatlantic_search(page, ArticleRequest())
