...
class _NoOpProcess:
    """Sentinel returned by launch_chrome when CDP_WS_URL is set (shared Chrome mode).
    Prevents verb scripts from terminating the app's shared Chrome process."""
    pid = -1
    returncode = 0
...
def launch_chrome(
    ...
    When CDP_WS_URL env var is set, a shared Chrome window is already running;
    returns a no-op sentinel so verb scripts don't spawn a second browser.
    """
    if os.environ.get("CDP_WS_URL"):
        return _NoOpProcess()
...
def wait_for_cdp_ws(port: int, timeout_s: float = 30.0) -> str:
    """Poll http://127.0.0.1:PORT/json/version until the WebSocket URL appears.
    When CDP_WS_URL env var is set, returns it immediately (shared Chrome mode)."""
    shared_url = os.environ.get("CDP_WS_URL")
    if shared_url:
        return shared_url
